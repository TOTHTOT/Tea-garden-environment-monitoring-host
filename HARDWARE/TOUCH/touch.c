...
void EXTI4_IRQHandler(void)
{
	...
	if(tp_dev.sta&TP_PRES_DOWN)			//触摸屏被按下
	{
		...
		{
			Show_Str(160,200,BLUE,WHITE,"保存中!!!",16,0);
			//写入数据到AT24C02
			...
			Show_Str(160,200,BLUE,WHITE,"保存完成!!",16,0);
			delay_ms(500);
			Show_Str(160,200,BLUE,WHITE,"          ",16,0);
			cursor_pos = 0;
			flag_num = 0;
			Rotate_Test();										//保存完成后返回主界面
		}
		//判断是否在第2页并显示光标位置(修改从机数量)
		...
		{
			cursor_pos = 1;
			LCD_ShowChar(SLAVE_ADDRESS_X+72,  SLAVE_ADDRESS_Y, BLUE, WHITE, ' ', 16, 0);						//从机编号光标显示
			LCD_ShowChar(SLAVE_ADDRESS_X+112, SLAVE_ADDRESS_Y, BLUE, WHITE, ' ', 16, 0);						//从机高地址光标显示
			LCD_ShowChar(SLAVE_ADDRESS_X+152, SLAVE_ADDRESS_Y, BLUE, WHITE, ' ', 16, 0);						//从机低地址光标显示
			LCD_ShowChar(SLAVE_ADDRESS_X+192, SLAVE_ADDRESS_Y, BLUE, WHITE, ' ', 16, 0);						//从机信道光标显示
			LCD_ShowChar(SLAVE_QUANTITY_X+72, SLAVE_QUANTITY_Y, BLUE, WHITE, '~', 16, 0);						//从机数量光标显示
			LCD_ShowChar(SCREEN_SLEEP_TIME_X+72, SCREEN_SLEEP_TIME_Y, BLUE, WHITE, ' ', 16, 0);					//屏幕休眠时间光标显示
			LCD_ShowChar(QUERY_INTERVAL_TIME_X+72, QUERY_INTERVAL_TIME_Y, BLUE, WHITE, ' ', 16, 0);				//轮询光标间隔显示
		}
		//判断是否在第2页并显示光标位置(修改休眠时间)
